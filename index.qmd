...
abcd
In script mode, R and Python code is written and saved in files, which can be executed as a whole to perform tasks or analyses. Interactive mode allows us to enter and execute commands line-by-line directly in an interactive shell or console, Command line mode involves running scripts from the terminal or command prompt.
...
salary <- read.table("02-data.tsv", sep = "\t", header = T)
Salary <- as.numeric(gsub("[^0-9.]","",salary$Salary))
